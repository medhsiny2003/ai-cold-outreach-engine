...
print("="*80)
print("   ⚡ SCAN TOTAL INSTANTANÉ DES REJETS GMAIL (ARCHITECTURES PROFESSIONNELLES)")
print("="*80)
...
# 2. Extract Bounces from [Gmail]/All Mail using lightweight header & body text
print("\n[*] Étape 2 : Extraction ultra-rapide des adresses rejetées...")
all_bounced_recipients = {}

mail.select('"[Gmail]/All Mail"', readonly=True)

# Search both Mailer-Daemon and Postmaster
...
for i in range(0, len(b_list), 50):
    chunk = b_list[i:i+50]
    seq = b",".join(chunk).decode('ascii')
    try:
        st_f, f_data = mail.fetch(seq, '(BODY.PEEK[HEADER] BODY.PEEK[TEXT])')
        if st_f != 'OK':
            continue
        ...
        for it in f_data:
            if isinstance(it, tuple) and len(it) > 1:
                ...
                # Regex match recipient from RFC headers & body
